import { KpiDataEntry } from "@/components/kpi-dashboard/kpi-data-entry"
...
export default async function KpiDataEntryPage({
  params,
}: {
  ...
  if (!userMayUpdateSegment(role, dept, sub, segment)) {
    redirect("/dashboard/kpi")
  }

  const mine = getKpiSegmentForProfile(dept, sub)
  if (mine && mine !== segment) {
    redirect(`/dashboard/kpi/data-entry/${mine}`)
  }

  return <KpiDataEntry segmentId={segment} />
}
